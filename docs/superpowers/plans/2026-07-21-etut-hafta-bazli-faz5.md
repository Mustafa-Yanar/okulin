...
> Karar (2026-07-21): hazırlık + final denetim bitince Mustafa'ya "kapılar yeşil, deploy ediyorum" son onayı sorulur; onay gelmeden 5.3'ten ileri GİDİLMEZ.

- [ ] **5.1 Canlı DRY-RUN:** `set -a; source .env.local; set +a; node scripts/migrate-etut-to-tables.mjs --reconcile` → rapor incele (soft-delete/cancel/conflict/ghost listeleri). Beklenmedik yıkıcı fark varsa DUR, araştır.
- [ ] **5.2 Ön kontroller:** `npm run build` + `npm run test` yeşil; Vercel env'ler tam; `git status` temiz.
- [ ] **5.3 MUSTAFA ONAYI:** dry-run özeti + kapı durumu sunulur → açık onay beklenir.
- [ ] **5.4 İndeksler:** `npx prisma db push` (yalnız 2 yeni indeks — eski kod etkilenmez, her an güvenli).
- [ ] **5.5 Reconcile #1:** `node scripts/migrate-etut-to-tables.mjs --reconcile --apply` → exit 0 + rapor temiz.
- [ ] **5.6 Merge + deploy:** `git checkout main && git merge --no-ff etut-hafta-bazli -m "feat(etüt): hafta-bazlı rezervasyon sistemi — Faz 1-5 cutover (SlotBooking'den bağımsız EtutSablon/EtutReservation tabloları)"` → `git push` → Vercel build'i izle (deployment READY olana dek).
- [ ] **5.7 Reconcile #2:** deploy READY olur olmaz `--reconcile --apply` tekrar — build penceresindeki (~2-5 dk) JSON yazımlarını yakalar (kural gereği post-deploy tablo yazımlarına dokunmaz; conflict raporu çıkarsa elle incele).
- [ ] **5.8 Canlı doğrulama:**
...
- [ ] **5.9 JSON cleanup:** `node scripts/cleanup-etut-json.mjs --apply` → yedek yolu kaydedilir (30 gün saklanır); doğrulama: rastgele 2 öğretmenin programTemplate'inde `etutSablonlari` anahtarı YOK, grid alanları DURUYOR.
- [ ] **5.10 Kapanış:** ledger + memory + Mustafa'ya rapor. Dal silinmez (referans), `etut-hafta-bazli` GitHub'da kalır.

**ROLLBACK PROSEDÜRÜ (pencere: cleanup sonrası ilk saatler):**
1. Kod: Vercel'de önceki production deployment'a instant rollback (dashboard → Promote); ardından `git revert -m 1 <merge-commit>` + push.
2. Veri: `node scripts/rollback-etut-json.mjs scripts/backups/etut-json-backup-<ts>.json --apply` — JSON geri gelir. **Cutover-sonrası tablo yazımları JSON'a DÖNMEZ** (kayıp kabul edilir — pencere bu yüzden kısa).
3. İndeksler/tablolar geri alınmaz (eski kod okumaz — zararsız).
