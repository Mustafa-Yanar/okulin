...
- [x] **Step 1: Failing testleri yaz** (`lib/etut/weeks.test.ts`)
...
- [x] **Step 2: FAIL doğrula** — Run: `npx vitest run lib/etut/weeks.test.ts` → Expected: FAIL (module not found)

- [x] **Step 3: Implementasyon** (`lib/etut/weeks.ts`)
...
- [x] **Step 4: PASS doğrula** — Run: `npx vitest run lib/etut/weeks.test.ts` → Expected: 11 test PASS
- [x] **Step 5: Commit**
...
- [x] **Step 1: Failing testleri yaz** (`lib/etut/overlap.test.ts`)
...
- [x] **Step 2: FAIL doğrula** — `npx vitest run lib/etut/overlap.test.ts` → module not found
- [x] **Step 3: Implementasyon** (`lib/etut/overlap.ts`)
...
- [x] **Step 4: PASS doğrula** — 9 test PASS
- [x] **Step 5: Commit**
...
- [x] **Step 1: Failing testleri yaz** (`lib/etut/level-pool.test.ts`)
...
- [ ] **Step 2: FAIL doğrula**
- [x] **Step 3: Implementasyon** (`lib/etut/level-pool.ts`)
...
- [x] **Step 4: HH:MM regex sıkılaştırma** — `scripts/etut-migration-lib.mjs` içindeki `validateSablon`'da `const HHMM = /^\d{2}:\d{2}$/;` satırını `const HHMM = /^([01]\d|2[0-3]):[0-5]\d$/;` yap. `scripts/etut-migration-lib.test.mjs`'e 1 test ekle: `validateSablon({...valid, start: '25:99'})` → `ok: false`.
- [x] **Step 5: PASS doğrula** — Run: `npx vitest run lib/etut/level-pool.test.ts scripts/etut-migration-lib.test.mjs` → 6 + 20 PASS
- [x] **Step 6: Commit**
...
- [ ] **Step 2: FAIL doğrula**
- [x] **Step 3: Implementasyon** (`lib/etut/reservations.ts`)
...
- [x] **Step 4: PASS doğrula** — Run: `npx vitest run lib/etut/reservations.test.ts` → 8 test PASS
- [x] **Step 5: Tip kontrolü** — Run: `npm run build` → Expected: `✓ Compiled successfully` (Prisma tipleriyle strict uyum; upsert where accessor `orgSlug_branch_sablonId_weekKey` Faz 1'de doğrulanmıştı)
- [x] **Step 6: Commit**

```bash
git add lib/etut/reservations.ts lib/etut/reservations.test.ts
git commit -m "feat(etüt-faz2a): EtutReservation veri katı — resolveEffective + tek-satır upsert + tombstone + advisory lock + 8 test"
```
...
- [x] **Step 1: Tam paket** — Run: `npm run build && npm test` → build yeşil; test toplamı 224 + 11 + 9 + 7 + 8 = **259 PASS** (sayı farklıysa nedenini raporla).
- [x] **Step 2: Plan checkboxlarını işaretle + commit**
