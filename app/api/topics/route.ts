...
// Bilinçli inline rol dallanması: erişim kapsamı isteğe (studentId) bağlı olduğundan
// withAuth mode'uyla ifade edilemez — öğrenci kendi id'sine sabitlenir, veli çocuğuna bakar.
...
export const POST = withAuth(async (req, _ctx, session) => {
  ...
  const { subject, topicIndex, percent } = parsed.data;
  ...
  const p = Math.max(0, Math.min(100, parseInt(String(percent)) || 0));
  const idx = String(parseInt(String(topicIndex)));

  const existing = await tdb().topic.findFirst({ where: { studentId } });
  const data = ((existing?.data as Record<string, Record<string, number>> | null) || {}); // data: Json
  if (!data[subject]) data[subject] = {};
  data[subject][idx] = p;
  if (existing) await tdb().topic.update({ where: { id: existing.id }, data: { data } });
  else await tdb().topic.create({ data: withScope({ studentId, data }) });
