...
export const POST = withAuth(['director'], async (req) => {
  ...
  await tdb().counselor.create({ data: withScope({ legacyId: id, name, username, passwordHash: hash, phone: normPhone, mustChangePassword: true }) });
...
export const PUT = withAuth(['director'], async (req) => {
  ...
  if (!a) return NextResponse.json({ error: 'Rehber bulunamadı' }, { status: 404 });
  ...
  await tdb().counselor.update({ where: { id: a.id }, data });
...
export const DELETE = withAuth(['director'], async (req, _ctx, session) => {
  ...
  await logAudit({
    ...
    target: { type: 'counselor', id, name: a?.name || id },
    detail: `Rehber silindi: ${a?.name || id}`,
  });
  return NextResponse.json({ ok: true });
});
