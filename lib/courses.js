...
export async function createCourse(ad) {
  ...
  const rec = { key, ad: name, core: false, family: null, active: true, seeded: true, createdAt: new Date().toISOString() };
...
export async function updateCourse(key, { ad, active }) {
  ...
  if (!rec) return { ok: false, status: 404, error: 'Ders bulunamadı' };
  const next = { ...rec };
  if (ad !== undefined) next.ad = String(ad).trim();
...
export async function seedCoursesFromConstants({ overwrite = false } = {}) {
  if (isSqlEnabled()) return { seeded: 0, skipped: true }; // SQL'de veri göçle hazır
