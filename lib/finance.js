...
// ── SQL sürümleri (bayrak açıkken). Redis sürümleriyle BİREBİR aynı kredilendirme mantığı.

// Makbuz no (SQL): TenantConfig.receiptCounter atomik artır.
export async function generateReceiptNoSql(orgOverride, branchOverride) {
...
export async function applyInstallmentPaymentSql(opts) {
  ...
  const payment = { id: Math.random().toString(36).slice(2, 10), date: paymentDate, amount: payAmount, method: method || 'Nakit', note: note || '', receiptNo, recordedBy: recordedBy || '' };
  ...
  await t.finance.update({ where: { id: record.id }, data: { payments } });
