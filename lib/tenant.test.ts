...
import { _scopedClient, prefixFor } from './tenant';
import { orgFromHost, resolveOrg } from './org';
...
// Çağrıları kaydeden sahte Upstash client (test sınırı: Redis'e cast kaçınılmaz).
function fakeClient(overrides: Record<string, Fn> = {}) {
  const calls: Call[] = [];
  const rec = (name: string) => (...args: unknown[]) => { calls.push([name, ...args]); return overrides[name]?.(...args); };
  return {
    calls,
    ...
    keys: (...a: unknown[]) => { calls.push(['keys', ...a]); return (overrides.keys || (async () => []))(...a); },
    scan: (...a: unknown[]) => { calls.push(['scan', ...a]); return (overrides.scan || (async () => ['0', []]))(...a); },
    pipeline: overrides.pipeline,
  };
}

const asRedis = (c: ReturnType<typeof fakeClient>) => c as unknown as Redis;
...
describe('_scopedClient — anahtar prefix', () => {
  it("get/set/del/exists/incr anahtarı prefix'ler", () => {
    const c = fakeClient();
    const r = _scopedClient(asRedis(c), PREFIX);
    r.get('teacher:x');
...
describe('_scopedClient — keys/scan prefix + strip', () => {
  ...
  it("scan match'i prefix'ler, bulunan anahtarları soyar", async () => {
    ...
    expect((scanCall?.[2] as { match?: string }).match).toBe(PREFIX + 'guidance:s1:*');
    expect(next).toBe('0');
...
describe('_scopedClient — pipeline', () => {
  it("pipeline get/set/del anahtarları prefix'ler, exec geçirir", async () => {
    const pcalls: Call[] = [];
    const fakePipe = {
      get: (...a: unknown[]) => pcalls.push(['get', ...a]),
      set: (...a: unknown[]) => pcalls.push(['set', ...a]),
      del: (...a: unknown[]) => pcalls.push(['del', ...a]),
      exec: async () => ['RESULT'],
    };
