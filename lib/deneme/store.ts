...
import { Prisma, type Exam, type ExamRow } from '@prisma/client';
import type { DenemeExam, DenemeRow } from './types';

export const dkeys = {
  exam: (id: string) => `deneme:exam:${id}`,
...
function sqlExamToObj(ex: (Exam & { rows?: ExamRow[] }) | null): DenemeExam | null {
  if (!ex) return null;
  return {
    id: ex.legacyId,
    name: ex.name,
    ...
    answerKey: (ex.answerKey || {}) as DenemeExam['answerKey'], // Json alanı
    rows: (ex.rows || []).map((r) => r.data as unknown as DenemeRow), // Json alanı — saveExam DenemeRow yazar
...
export async function getExam(id: string): Promise<DenemeExam | null> {
...
export async function saveExam(exam: DenemeExam): Promise<void> {
  const cols = {
    ...
    subjectKeys: exam.subjectKeys || [], answerKey: exam.answerKey ?? Prisma.DbNull,
  ...
  if (ex) {
    ...
    ex = await tdb().exam.create({ data: withScope({ legacyId: exam.id, ...cols, createdAt: exam.createdAt ? new Date(exam.createdAt) : new Date() }) });
  }
  const rows = Array.isArray(exam.rows) ? exam.rows : [];
  if (rows.length) {
    await tdb().examRow.createMany({ data: rows.map((r) => ({ examId: ex.id, studentId: r.studentId || null, data: r as object })) });
  }
}
...
export async function addExamToIndex(): Promise<void> {}
...
export async function getNameMap(): Promise<Record<string, string>> {
  ...
  return (cfg?.denemeNameMap || {}) as Record<string, string>; // Json alanı — saveNameMap yazar
}

export async function saveNameMap(map: Record<string, string>): Promise<void> {
  const cfg = await tdb().tenantConfig.findFirst();
  if (cfg) await tdb().tenantConfig.update({ where: { orgSlug_branch: { orgSlug: cfg.orgSlug, branch: cfg.branch } }, data: { denemeNameMap: map } });
  else await tdb().tenantConfig.create({ data: withScope({ denemeNameMap: map }) });
}

// Bir öğrencinin tüm denemelerdeki sonuç noktalarını üret (eskiden yeniye).
// /api/deneme/me ve /api/deneme/student ortak kullanır.
export async function buildStudentPoints(studentId: string) {
  ...
  for (const exam of exams) {
    ...
    points.push({
      ...
      fullDate: new Date(exam.date as string).toLocaleDateString('tr-TR'),
