...
import { sliceFlat, sliceExam, getTemplate } from './template.js';
import { gradeExam, toplamNet } from './score.js';

// exam.answerKey[kitapcik] (boxesRaw) → { dersKey: anahtarDizisi }.
export function keyToSubjects(exam, kitapcik = 'A') {
...
export function hasAnswerKey(exam, kitapcik = 'A') {
...
export function gradeFlat(exam, flatAnswers, kitapcik = 'A') {
  ...
  return { results, toplamNet: toplamNet(results) };
}
