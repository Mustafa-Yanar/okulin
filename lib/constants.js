...
// ── SLOT MODELİ (2026-07: 7-gün bağımsız) ────────────────────────────────────
// Eskiden hafta içi (w1-w12) + hafta sonu (e1-e12) iki sabit dizi vardı; her gün
// tipi 12 slot paylaşırdı. Yeni model: HER GÜN bağımsız — kendi ders SAYISI ve
// kendi SAATLERİ. Slot id'leri güne özgü: `d{gün}s{n}` (gün 0-6, n 1-tabanlı).
//   d0s1 = Pazartesi 1. ders · d5s3 = Cumartesi 3. ders
// Saatler + günlük ders sayısı TenantConfig.slotTimes'ta saklanır (lib/slots.js).

// Varsayılan günlük ders sayısı (kurum config girmezse).
export const DEFAULT_SLOTS_PER_DAY = 12;
// Slot sayısı üst sınırı (UI + validasyon).
...
// Eski (w/e) slot id'sini yeni (d{gün}s{n}) id'ye çevir — MİGRASYON + geriye uyum.
...
// GERİYE UYUM: eski sabit dizileri koruyoruz (henüz göç etmemiş dosyalar için).
// Yeni kod slotIdsForDay/slotId kullanmalı. Bunlar kaldırılacak (deprecated).
...
// Hafta içi sadece mezun sınıfların ders olarak atanabileceği ilk 6 slot (deprecated id listesi)
export const MEZUN_ONLY_LESSON_SLOTS = ['w1','w2','w3','w4','w5','w6'];
// Yeni model: hafta içi ilk N slot mezun-only (slot NUMARASINA göre, güne özgü id'de geçerli).
export const MEZUN_ONLY_LESSON_COUNT = 6;

// Mezun öğrencilerin etüt rezervasyonu yapamayacağı hafta içi slot (deprecated id)
export const MEZUN_FORBIDDEN_ETUT_SLOT = 'w9';
// Yeni model: hafta içi bu slot NUMARASINA (9.) mezun etüt yasak.
...
// Tüm sınıflar tek düz dizi (ortaokul + lise + mezun). Ön eşleştirme / program araçları kullanır.
...
// Veli-öğrenci yakınlık dereceleri (veli paneli). "Diğer" daima sonda.
export const PARENT_RELATIONS = [
  'Anne', 'Baba', 'Anneanne', 'Babaanne', 'Dede',
  'Abla', 'Ağabey', 'Amca', 'Dayı', 'Hala', 'Teyze', 'Vasi', 'Diğer',
];
